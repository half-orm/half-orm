...
class Test(TestCase):
    def test_threads_get_distinct_connections(self):
        "Each thread must receive its own psycopg connection object."
        conns = []
        lock = threading.Lock()
    ...
    def test_thread_disconnect_does_not_affect_others(self):
        ...
        def thread_a():
            # Establish a connection, then disconnect
            _ = model._connection
            barrier.wait()          # sync: both threads have a connection
            model.disconnect()

        def thread_b():
            _ = model._connection   # ensure B has its own connection
            barrier.wait()          # sync: A is about to disconnect
            # Give A a moment to complete disconnect
        ...
        # Restore main-thread connection (it was not touched, but re-open A's)
        model.reconnect()
    ...
    def test_ping_reconnects_current_thread(self):
        ...
        def do_ping():
            # Force-close the thread-local connection
            model._connection.close()
            # ping() must detect the closed state and reconnect
            model.ping()
            return model.execute_query("select 1").fetchone()
    ...
    def test_transaction_state_isolated_per_thread(self):
        ...
        def enter_transaction(name):
            with Transaction(model):
                ...
                barrier.wait()   # both threads inside Transaction simultaneously
        ...
        # Each thread independently sees level == 1, not 2
