...
class TestFKDelete(TestCase):
    ...
    # ------------------------------------------------------------------
    # SQL shape — verify subquery is used
    # ------------------------------------------------------------------

    def test_delete_via_fk_sql_uses_subquery(self):
        "DELETE through FK navigation must use IN (SELECT ...) not fkey.values()"
        post = halftest.person_cls(last_name='fk_write_tester_A').post_rfk()
        pk_names = list(post._ho_pkey.keys())
        sub_sql, _ = post._ho_prep_select(*pk_names)
        self.assertIn('select', sub_sql.lower())
        self.assertIn('join', sub_sql.lower())
